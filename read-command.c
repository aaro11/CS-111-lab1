...
void addToken_make(token*** start, token*** end, tokentype t, char* tempchars, int cwp, bool paren) //copy memory, paren is no longer used
{
	...
	for (i=0; i < cwp; i++)
		tokchars[i] = tempchars[i];
	tokchars[i] = ' ';
	int clistpos;
	if (cwp > 0)
	{
		tokchars = malloc(cwp+1);
		for (i=0; i < cwp; i++)
			tokchars[i] = tempchars[i];
		tokchars[i] = ' ';
		TOT_SIZE = cwp+1;
...
token* infixToPostfix(token* start)
{
	...
	for (; start != NULL; start = start->next)
		...
		else if (start->t == SEMICOLON_TOKEN)
		{
			while (stack_end && stack_end->t != LEFT_PAREN_TOKEN)
...
command_stream* evaluatePostfix(token* finalTokenStream)
{
	...
	*/
	...
	for (curr = finalTokenStream; curr != NULL; curr = curr->next)
		...
		else if (curr->t == SEMICOLON_TOKEN) //stop working on the current stream and instead make a new stream
		{
...
//  return 0;
	...
	if (!noIllegalChars(buf) || !parenmatch(buf) || !redirOrderValid(buf))	
		error (1, 0, "illegal command syntax");
	...
	for (clean = start; clean; clean = clean->next) //start by removing adjacent newlines
		if (clean->t == NEWLINE_TOKEN)
			while (clean->next && clean->next->t == NEWLINE_TOKEN)
			{	
				clean->next = clean->next->next;
				clean->next->prev = clean;
			}
	for (clean = start; clean; clean = clean->next)
		if (clean->t == NEWLINE_TOKEN)
		{
			if ((clean->prev != NULL && clean->prev->t != WORD_TOKEN && clean->prev->t != RIGHT_PAREN_TOKEN) || (clean->next != NULL && clean->next->t != WORD_TOKEN && clean->next->t != RIGHT_PAREN_TOKEN)) //function as whitespace
			{
				token* p = clean->prev;
				token* n = clean->next;
				p->next = n;
